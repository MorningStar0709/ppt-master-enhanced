...
3. 验证环境：
   ...
   看到 `Python 3.12.x`（或任意 3.10+ 版本）即可。
...
> 如果 `pip` 无法识别，用 `conda run -n ppt-master python -m pip install -r requirements.txt`。
...
|--------|-----------------|---------|------|
| **CairoSVG** — 更高质量 PNG 后备图 | 你希望在不原生支持 SVG 的 Office 版本下获得更清晰的 PNG 后备图。`svglib`（已默认安装）足够大多数场景。 | 安装 [GTK3 Runtime](https://github.com/nickvdp/gtk3/releases) 后 `conda run -n ppt-master pip install cairosvg` | `conda run -n ppt-master python -c "import cairosvg"` |
...
**方法 3** — 在目标环境里验证解释器：`conda run -n ppt-master python --version`。

### `conda run -n ppt-master pip install` 报权限错误
...
### `conda run -n ppt-master pip install` 网络问题

```powershell
# 清华镜像（国内推荐）
conda run -n ppt-master pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple
...
`pip` 装到了另一个 Python 环境。用 `conda run -n ppt-master python -m pip install -r requirements.txt` 确保对应同一个。

### `import fitz` 失败

1. 升级 pip：`conda run -n ppt-master python -m pip install --upgrade pip`
2. 预编译包：`conda run -n ppt-master pip install PyMuPDF --only-binary :all:`
