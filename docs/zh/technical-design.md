...
为了确保 AI 在生成复杂演示文稿时的稳定性和可靠性，本项目建立了一套严格的执行纪律（Execution Discipline），详细规则参见 `AGENTS.md` 和 `CLAUDE.md`。核心要点如下：

1. **严格串行工作流**：步骤必须按顺序执行（1.源处理 → 2.项目初始化 → 3.模板选择 → 4.策略师阶段 → 5.图片生成 → 6.执行师生成 → 7.SVG审查 → 8.导出）。
2. **强制交互检查点**：在 Step 3（模板选择）、Step 4（八项确认）和 Step 7（SVG 终审）必须停下来等待用户明确确认，禁止 AI 代替用户做决定。
3. **主 Agent 全程负责**：Executor 阶段的页面生成必须由主 Agent 连续完成，禁止将页面生成任务拆分给子 Agent，以确保跨页面的设计一致性。
4. **禁止 Shell 重定向写入**：在生成 SVG、讲稿或审查文档时，禁止使用 PowerShell 的 `>` 或 `Out-File` 等重定向指令，必须通过 Python 脚本或 IDE 编辑工具写入，以防止 Windows 编码问题导致的损坏。
5. **Conda 环境隔离**：所有 Python 操作必须通过 `conda run -n ppt-master` 启动。
