...
def get_png_renderer_info() -> tuple[str | None, str, str | None]:
    ...
    if PNG_RENDERER == 'cairosvg':
        ...
        return ('svglib', '(some gradients may be lost)',
                'Install cairosvg for better results: conda run -n ppt-master pip install cairosvg')
    else:
        return (None, '(not installed)',
                'Install via: conda run -n ppt-master pip install cairosvg or conda run -n ppt-master pip install svglib reportlab')
