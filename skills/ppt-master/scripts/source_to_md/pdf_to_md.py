...
try:
    ...
    print("[ERROR] PyMuPDF not installed. Run: conda run -n ppt-master pip install PyMuPDF", file=sys.stderr)
    sys.exit(1)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='''
Examples:
  conda run -n ppt-master python pdf_to_md.py book.pdf                    # Convert a single file
  conda run -n ppt-master python pdf_to_md.py book.pdf -o output.md      # Specify output file
  conda run -n ppt-master python pdf_to_md.py ./pdfs                      # Convert all PDFs in directory
  conda run -n ppt-master python pdf_to_md.py ./pdfs -o ./markdown       # Specify output directory
