...
def _convert_docx(input_file: Path, out_file: Path) -> str:
    try:
        ...
        print("[ERROR] mammoth not installed. Run: conda run -n ppt-master pip install mammoth")
        return ""
...
def _process_html_images(html: str, base_dir: Path, media_dir: Path, rel_media_dir: str) -> str:
    ...
    try:
        ...
        print("[ERROR] beautifulsoup4 not installed. Run: conda run -n ppt-master pip install beautifulsoup4")
        return html
...
def _convert_html(input_file: Path, out_file: Path) -> str:
    try:
        ...
        print("[ERROR] markdownify not installed. Run: conda run -n ppt-master pip install markdownify")
        return ""
...
def _convert_epub(input_file: Path, out_file: Path) -> str:
    try:
        ...
        print(f"[ERROR] Missing dependency: {e.name}. "
              f"Run: conda run -n ppt-master pip install ebooklib markdownify beautifulsoup4")
        return ""
...
def _convert_ipynb(input_file: Path, out_file: Path) -> str:
    try:
        ...
        print("[ERROR] nbconvert not installed. Run: conda run -n ppt-master pip install nbconvert")
        return ""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  conda run -n ppt-master python doc_to_md.py lecture.docx                # Word → Markdown (mammoth)
  conda run -n ppt-master python doc_to_md.py article.html                # HTML → Markdown (markdownify)
  conda run -n ppt-master python doc_to_md.py book.epub                   # EPUB → Markdown (ebooklib)
  conda run -n ppt-master python doc_to_md.py notebook.ipynb              # Jupyter → Markdown (nbconvert)
  conda run -n ppt-master python doc_to_md.py manuscript.tex              # LaTeX → Markdown (pandoc fallback)
