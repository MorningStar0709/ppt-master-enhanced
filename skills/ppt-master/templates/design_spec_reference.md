...
- **Usage method**: Record icon names using the final placeholder contract `<library>/<icon-name>` (for example, `chunk/checkmark` or `tabler-filled/home`)

### Recommended Icon List (fill as needed)

| Purpose | Icon Path | Page |
| ------- | --------- | ---- |
| [example] | `chunk/checkmark` | Slide XX |
...
Use `references/shared-standards.md` as the source of truth for SVG constraints and PPT compatibility.

The design spec only needs to restate project-specific technical choices such as:

1. viewBox: `{canvas_info['viewbox']}`
2. any image clipping decision that relies on `shared-standards.md` §1.2
3. any marker/arrow decision that relies on `shared-standards.md` §1.1
