...
| ------------------ | --------------------------------------- |
| **Clipping/Masking** | `clipPath` only on `<image>` per `shared-standards.md` §1.2; `mask` forbidden |
...
1. **Apply Template**: Use `project_manager.py apply-template <project_path> psychology_attachment`
