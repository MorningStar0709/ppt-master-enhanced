...
5. `clipPath` is conditionally allowed only on `<image>` elements per `references/shared-standards.md` §1.2; `mask`, `<style>`, `class`, and `foreignObject` remain forbidden
...
1. **Template Deployment**: Use `project_manager.py apply-template <project_path> 中汽研_常规`.
