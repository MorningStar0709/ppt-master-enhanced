...
4. **Forbidden**: No `mask`; `clipPath` is only allowed on `<image>` elements per `references/shared-standards.md` §1.2.

### Forbidden Elements (Blacklist)

- `clipPath` on `<image>` is conditionally allowed per `references/shared-standards.md` §1.2; `mask` remains forbidden (clipping/masking)
