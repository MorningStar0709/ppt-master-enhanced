...
If you are debugging manually, do not rely on Unix-only shell snippets; use the IDE/file tools or a platform-appropriate copy command.
...
6. Validate the template directory with `conda run -n ppt-master python scripts/svg_quality_checker.py templates/layouts/<template_name> --format ppt169`
...
Keep template-local technical rules concise: use `references/shared-standards.md` as the source of truth, and only add template-specific deviations or reminders inside each template's `design_spec.md`.
...
| `clipPath` on non-image elements | Redesign layout |
