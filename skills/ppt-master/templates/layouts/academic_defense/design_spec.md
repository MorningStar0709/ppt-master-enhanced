...
5. `clipPath` is conditionally allowed only on `<image>` elements per `references/shared-standards.md` §1.2; `mask`, `<style>`, `class`, and `foreignObject` remain prohibited
...
1. Apply the template with `project_manager.py apply-template <project_path> academic_defense`
