...
- `clipPath` on `<image>` is conditionally allowed per `references/shared-standards.md` §1.2; `mask` remains forbidden
...
1. Apply the template with `project_manager.py apply-template <project_path> google_style`
