...
2. No `mask`, `<style>`, `class`, `foreignObject`, `textPath`, or animation tags; `clipPath` is allowed only on `<image>` elements per `references/shared-standards.md` §1.2
...
1. Apply the template with `project_manager.py apply-template <project_path> 招商银行`
