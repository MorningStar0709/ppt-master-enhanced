...
|------|----------|-------------|
| New template ID | Yes | Template directory / index key. It MUST be an English-only ASCII slug such as `my_company` and must match `layouts_index.json` exactly |
...
Use platform-native file listing commands in this workflow. Do NOT introduce Unix-only shell assumptions such as `ls -la` when running in Windows/PowerShell environments.
...
`design_spec.md` should keep shared SVG/PPT rules concise and defer to `references/shared-standards.md` unless the template needs an explicit exception or reminder.
