...
All outputs MUST be placed under the `projects/` directory as **standalone research materials**, not as an initialized PPT project:
...
>
> **Project-boundary rule**: `projects/<topic_name>.md` is a research artifact, not a Step 2 project path. If these materials later enter the main PPT workflow, the agent MUST still run `project_manager.py init ...` to create the real project directory, then import the research outputs with `project_manager.py import-sources --copy`.
...
Do **not** treat this Markdown file itself as the initialized PPT project. It is an input artifact for later import.
...
Create an image folder under `projects/<topic_name>/`, then download images with descriptive filenames into that directory.
...
> From here, the user or main pipeline can use these materials as input for PPT generation. Preferred handoff:
> 1. `project_manager.py init <project_name> --format <format> --dir projects`
> 2. reuse the exact printed `<project_path>`
> 3. `project_manager.py import-sources "<project_path>" "projects/<topic_name>.md" "projects/<topic_name>/" --copy`
...
- Do not skip Step 2 project initialization later just because the research outputs already live under `projects/`; the main workflow still requires a real `projects/<project_name>_<format>/` directory created by `project_manager.py init`
