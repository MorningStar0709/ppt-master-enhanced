...
- `MUST`: non-skippable action
- `DECISION REQUIRED`: Strategist must choose or present a valid option
- `BLOCKING`: user confirmation required before continuing
...
If Step 3 selected **using an existing template**, verify that the project template assets are actually present before continuing:
- expected minimum: `templates/design_spec.md` plus at least one `templates/*.svg`
- if they are missing or incomplete, stop and run `project_manager.py apply-template <project_path> <template_name>` first
- do not silently continue into Strategist while assuming a template has already been applied

🚧 **GATE — Mandatory read before rendering the Eight Confirmations checkpoint**:
```
read_file references/interactive-checkpoint-guidelines.md
```
The checkpoint card MUST follow that document exactly. Do not improvise alternate titles, prompts, option labels, or grouped-card structures.
...
> **Interactive checkpoint rule**: When an interactive question/choice tool is available, use it for the Eight Confirmations instead of scattering plain text `A/B/C` prompts across multiple turns.
>
> **Preferred card design**: Present this step as one bundled checkpoint, not grouped pseudo-multi-select cards.
> - Main card title: `8项确认`
> - Main card prompt: `请确认以下 8 项设计参数（可直接使用推荐值）：`
> - Main card body: list all 8 recommended parameters explicitly
> - Main card actions: `确认全部推荐（推荐）` and `我想调整部分参数`
> - Optional second card: `其他补充`
> - Optional second card prompt: `是否有更多的补充信息需要提供？（可选）`
> - The title, prompt, and option labels are fixed workflow strings; only the 8 recommended parameter values may vary.
> Do not use grouped category cards like “画布格式与页数 / 设计风格与色系 / 图标与排版 / 图片方案” as the primary confirmation UI.

> **Execution discipline**: This is the second blocking checkpoint in the pipeline (after template selection, before the SVG Review Gate approval). Once the user confirms, the AI must automatically complete the Design Specification & Content Outline and seamlessly proceed to subsequent non-blocking steps — image generation (if applicable), SVG generation, and review preparation — without adding extra questions in between.
...
> 2. Search for icon availability with minimal scope:
>    - Preferred: `conda run -n ppt-master python scripts/asset_lookup.py icons <keyword> --library <chosen-library>`
>    - Fallback: use IDE/file search in `templates/icons/<chosen-library>/`
> 3. Use the verified filename (without `.svg`) as the icon name
> 4. Always include the library prefix and store the final contract as `<library>/<icon-name>` (e.g., `chunk/home` or `tabler-filled/home`)
> 5. List the final icon inventory and chosen library in the Design Spec; Executor may only use icons from this list
>
> **Do NOT preload any full icon catalog** — use targeted lookup only.
...
**When selection includes B**, you must run `conda run -n ppt-master python scripts/analyze_images.py <project_path>/images` before outputting the spec, and integrate scan results into the image resource list.
...
> **Pipeline handoff**: When C) AI generation is selected, proceed directly to Image_Generator as the next non-blocking workflow step after the design spec is complete. Do not add an extra user confirmation point unless the user explicitly asks to review image prompts first.

### Visualization Reference (Non-blocking — Strategist recommends, no user confirmation needed)

**Execution type**: `CONDITIONAL`

When content outline pages involve **data visualization or infographic-style structured information design** (comparisons, trends, proportions, KPIs, flows, timelines, org structures, strategic frameworks, etc.), Strategist MUST select appropriate visualization types from the built-in template library or explicitly mark the page as `custom layout`.

> **Mandatory first step**: Do **not** read the full `templates/charts/charts_index.json` by default. Start with a targeted lookup based on page intent or keywords, then read only the matched candidate entries.

> **Selection workflow**:
> 1. For each page in the content outline, determine whether it needs visualization based on its information structure
> 2. Run a targeted lookup:
>    - Preferred: `conda run -n ppt-master python scripts/asset_lookup.py charts <keyword>`
>    - Fallback: read only the relevant `quickLookup` branch and the matched chart entries in `templates/charts/charts_index.json`
> 3. Compare the matched candidates by `summary` / `keywords` to find the best fit
> 4. Read the referenced SVG template only when the selected visualization needs closer structural reference
...
3. Save to: `<project_path>/design_spec.md` using the exact project path already created upstream
