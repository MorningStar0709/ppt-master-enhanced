...
AI image generation may run only when image-generation capability is actually available.

### Capability requirements

- a selected backend is known (`IMAGE_BACKEND` or `--backend`)
- the backend is supported by the tool
- provider-specific credentials are present
- the backend can be loaded in the current environment

If any of the above is missing, automatic AI generation is **not available** for the current run.

### Hard rule

Do not treat AI generation as an always-available fallback.

If the workflow wants an AI-generated asset but no usable backend exists, the agent must switch to one of these non-AI fallback paths:

- verified local substitute
- external sourcing when allowed by workflow context
- user manual generation using `images/image_prompts.md`
- labeled placeholder
- layout redesign that removes hard dependency on the missing image

Missing backend capability is not a valid reason to leave a required image region silently blank.
...
```bash
conda run -n ppt-master python scripts/image_gen.py "your prompt" \
  --backend <provider> --aspect_ratio 16:9 --image_size 1K \
...
| `--backend` | `-b` | Override backend (`gemini`/`openai`/`minimax`/`stability`/`bfl`/`ideogram`/`qwen`/`zhipu`/`volcengine`/`siliconflow`/`fal`/`replicate`) | None |
| `--model` | `-m` | Model name | Backend default |
| `--list-backends` | - | Print support tiers and exit | `false` |

**Configuration sources**:
- Current process environment variables
- Repository-root `.env` as fallback

Precedence:
- Current process environment wins
- Repository-root `.env` fills missing values

| Variable | Required | Description |
|----------|----------|-------------|
| `IMAGE_BACKEND` | Required | `gemini` / `openai` / `minimax` / `stability` / `bfl` / `ideogram` / `qwen` / `zhipu` / `volcengine` / `siliconflow` / `fal` / `replicate` |
...
> `.trae/.env` is not a supported config path.
...
> Recommended invocation on Windows/Trae: pass `--backend <provider>` explicitly when running a generation command. This avoids ambiguity when the terminal session has stale environment variables.

**Provider selection rule**:

- Do not assume any provider is the default for all users
- Use the provider declared by `IMAGE_BACKEND`, or pass `--backend <provider>` explicitly
- Keep prompts and workflow examples provider-neutral unless a provider-specific capability is the point of the example
- If no usable provider is configured, stop automatic AI generation and move to a non-AI fallback instead of speculative provider retries

**Common provider examples**:

| Provider placeholder | Typical env vars | Notes |
|----------------------|------------------|-------|
| `<provider>=gemini` | `IMAGE_BACKEND=gemini`, `GEMINI_API_KEY`, `GEMINI_MODEL` | Core backend |
| `<provider>=openai` | `IMAGE_BACKEND=openai`, `OPENAI_API_KEY`, `OPENAI_MODEL` | Core backend |
| `<provider>=qwen` | `IMAGE_BACKEND=qwen`, `QWEN_API_KEY`, `QWEN_MODEL` | Core backend |
| `<provider>=zhipu` | `IMAGE_BACKEND=zhipu`, `ZHIPU_API_KEY`, `ZHIPU_MODEL` | Core backend |
| `<provider>=volcengine` | `IMAGE_BACKEND=volcengine`, `VOLCENGINE_API_KEY`, `VOLCENGINE_MODEL` | Core backend |
| `<provider>=minimax` | `IMAGE_BACKEND=minimax`, `MINIMAX_API_KEY`, `MINIMAX_MODEL` | Experimental backend |
| `<provider>=stability` | `IMAGE_BACKEND=stability`, `STABILITY_API_KEY`, `STABILITY_MODEL` | Extended backend |
| `<provider>=ideogram` | `IMAGE_BACKEND=ideogram`, `IDEOGRAM_API_KEY`, `IDEOGRAM_MODEL` | Extended backend |

**Support tiers (recommended usage)**:

| Tier | Providers | Guidance |
|------|-----------|----------|
| Core | `gemini`, `openai`, `qwen`, `zhipu`, `volcengine` | Prefer for everyday PPT generation |
| Extended | `stability`, `bfl`, `ideogram` | Use when their image style is specifically needed |
| Experimental | `minimax`, `siliconflow`, `fal`, `replicate` | Use with explicit confirmation of provider availability |
...
**Configuration preflight (recommended)**:

Before the first generation attempt in a session:

```bash
conda run -n ppt-master python scripts/image_gen.py --list-backends
conda run -n ppt-master python scripts/image_gen.py --check-capability
```

Interpretation rule:

- `--list-backends` confirms tool support tiers, not credential readiness by itself
- `--check-capability` checks whether the selected backend is actually usable in the current environment without running a real generation job
- a real generation attempt is allowed only after backend selection and credential readiness are known
- if readiness is unknown or absent, do not present AI generation as a stable automatic fallback

When calling a real generation command, prefer an explicit backend:

```bash
conda run -n ppt-master python scripts/image_gen.py "your prompt" --backend <provider> --aspect_ratio 16:9 --image_size 1K --output project/images
```

Provider examples:

```bash
conda run -n ppt-master python scripts/image_gen.py "your prompt" --backend gemini --aspect_ratio 16:9 --image_size 1K --output project/images
conda run -n ppt-master python scripts/image_gen.py "your prompt" --backend openai --aspect_ratio 16:9 --image_size 1K --output project/images
conda run -n ppt-master python scripts/image_gen.py "your prompt" --backend qwen --aspect_ratio 16:9 --image_size 1K --output project/images
```
...
3. Remove watermark: `conda run -n ppt-master python scripts/gemini_watermark_remover.py <image_path>`
...
- the workflow wants an image but no usable backend is configured locally
- the user did not configure AI image generation in the current environment
- automatic provider invocation is unstable or unavailable
...
- In pipeline mode, remember that this phase only guarantees asset readiness. It does **not** prove that the generated images have already been inserted into SVG pages or approved by the user.
...
Executor obligations after handoff:

- verify that every image marked `Generated` is actually consumed by the intended page(s), or explicitly mark it unused with justification
- do not assume `images/` presence alone means visual integration is complete
- if the page still shows a blank placeholder or visually missing region, treat that as an Executor integration issue, not as a completed image-generation success
...
### Downstream Integration Reminder

- [ ] Executor has a clear mapping of which generated image belongs to which page
- [ ] Pipeline users are not told to approve export before the inserted images are visibly confirmed in deck review
- [ ] If no usable backend existed, the fallback path used (manual generation / external asset / placeholder / layout redesign) is explicit
