...
4. Preview: conda run -n ppt-master python -m http.server -d <project_path> 8000 → /svg_output/<filename>.svg
...
```bash
conda run -n ppt-master python -m http.server -d <project_path> 8000
...
```bash
conda run -n ppt-master python scripts/finalize_svg.py <project_path>         # Icons, images, text, rounded rects — all in one pass
conda run -n ppt-master python scripts/svg_to_pptx.py <project_path> -s final  # Export PPTX from final version
```
...
```bash
conda run -n ppt-master python scripts/svg_finalize/embed_images.py <svg_file>                         # Single file
conda run -n ppt-master python scripts/svg_finalize/embed_images.py <project_path>/svg_output/*.svg    # Batch
conda run -n ppt-master python scripts/svg_finalize/embed_images.py --dry-run <project_path>/svg_output/*.svg  # Preview
```
...
### Rounded Corner Handling (clipPath Conditionally Allowed)

`clip-path` is allowed **only on `<image>` elements** when it follows `references/shared-standards.md` §1.2. Use it for circular or rounded image crops when you need native PPT picture geometry.

Do not use `clipPath` on shapes or groups. When the target is not an `<image>`, redesign the geometry directly instead.

Alternatives when clip-based image cropping is not suitable:
...
Browser security policy blocks cross-directory requests. Start an HTTP server from the project root. During Step 7 review, first run `review_manager.py verify` so it prepares the unified review SVG source under `review/preview_finalized/`, then inspect `review/preview_deck.html`. Reserve normal `finalize_svg.py` + `svg_final/` for the post-approval export path.
