...
- **Path portability rule**: Image `href` values MUST stay project-relative (`../images/...`, `./assets/...`, etc.). Absolute local filesystem paths such as `D:\...`, `C:\...`, `\\server\share\...`, or `file:///...` are forbidden because they break after project migration and in review/export copies
...
- **`<use>` boundary rule**: For icons, only `data-icon="library/name"` is allowed. Do **not** use `<use href="tabler-filled/name.svg#icon">`, `<use xlink:href="...">`, or any external/file-based icon reference — post-processing will not embed them and PPT export can lose the icon. Plain `<use href="#localId">` is allowed only for same-file reuse of locally defined SVG elements.
...
Must be executed in order in the **main workflow**:
...
# Timestamp here is export-history metadata only; project directories stay stable and untimestamped.
```
...
- In the main workflow, do NOT add extra flags like `--only` unless the user explicitly requests a debug or single-mode export
