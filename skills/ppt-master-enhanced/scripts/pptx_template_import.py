...
def main() -> int:
    ...
    if not pptx_path.exists():
        safe_print(f"Error: file does not exist: {pptx_path}")
        return 1
    if pptx_path.suffix.lower() != ".pptx":
        safe_print(f"Error: expected a .pptx file, got: {pptx_path.name}")
        return 1
    ...
    if args.skip_manifest and args.manifest_only:
        safe_print("Error: --skip-manifest and --manifest-only cannot be used together")
        return 1
    ...
    if not args.skip_manifest:
        try:
            ...
            safe_print(f"Error: failed to extract PPTX metadata: {exc}")
            return 1
    ...
    if args.manifest_only:
        safe_print(f"Imported PPTX template source: {pptx_path.name}")
        safe_print(f"Output directory: {output_dir}")
        if not args.skip_manifest:
            safe_print(f"Manifest: {manifest_path.name}")
            safe_print(f"Assets exported: {len(manifest['assets']['allAssets'])}")
            safe_print(f"Common assets: {len(manifest['assets']['commonAssets'])}")
            safe_print(f"Slides analyzed: {len(manifest['slides'])}")
        return 0
    ...
    try:
        ...
        safe_print(f"Error: failed to export PPTX slides to SVG: {exc}")
        return 1

    if args.no_externalize:
        safe_print(f"Export mode: {export_mode}")
        safe_print(f"Exported raw SVG slides: {len(raw_svg_files)}")
        safe_print(f"Output directory: {output_dir}")
        return 0
    ...
    if not args.no_optimize:
        ...
        safe_print(f"Optimized SVG files: {len(optimize_results)}")
        safe_print(f"SVG bytes after structural optimization: {before_opt} -> {after_opt}")
        safe_print(f"Icon candidate report: {optimize_output_dir / 'icon_candidates.json'}")
    ...
    safe_print(f"Export mode: {export_mode}")
    safe_print(f"Exported SVG slides: {len(results)}")
    safe_print(f"Inline images externalized: {total_images}")
    safe_print(f"SVG bytes: {total_before} -> {final_svg_bytes}")
    safe_print(f"Reference SVG selection: {output_dir / 'reference_svg_selection.json'}")
    safe_print(f"Output directory: {output_dir}")
    return 0
