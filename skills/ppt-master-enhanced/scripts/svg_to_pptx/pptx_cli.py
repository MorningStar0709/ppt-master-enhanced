...
try:
    ...
    from review_utils import get_review_gate_status  # type: ignore
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=f'''
Examples:
    %(prog)s examples/ppt169_demo -s final    # Default: timestamped export snapshots saved to exports/
    %(prog)s examples/ppt169_demo --only native   # Advanced/debug: only native shapes version
    %(prog)s examples/ppt169_demo --only legacy   # Advanced/debug: only SVG image version
...
Compatibility mode (enabled by default):
    ...
    - Requires svglib: conda run -n ppt-master pip install svglib reportlab
...
Output naming rule:
    - Project directories remain stable and do not use timestamps
    - Default exports under exports/ do use timestamps because each export is a historical snapshot
''',
    )
    ...
    if not review_status.export_allowed:
        print("Error: SVG Review Gate has not passed. PPT export is blocked.")
        for blocker in review_status.blockers:
            print(f"  - {blocker}")
        print("Check review status with:")
        print(f"  conda run --no-capture-output -n ppt-master python scripts/review_manager.py status \"{project_path}\" --compact")
        print(f"  conda run --no-capture-output -n ppt-master python scripts/review_manager.py verify \"{project_path}\" --compact")
        print(f"  conda run --no-capture-output -n ppt-master python scripts/review_manager.py repair-focus \"{project_path}\"")
        sys.exit(1)
