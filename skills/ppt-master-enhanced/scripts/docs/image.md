...
conda run -n ppt-master python scripts/image_gen.py "Abstract tech background" --aspect_ratio 16:9 --image_size 4K
conda run -n ppt-master python scripts/image_gen.py "Concept car" -o projects/demo/images
conda run -n ppt-master python scripts/image_gen.py "Beautiful landscape" -n "low quality, blurry, watermark"
...
```bash
conda run -n ppt-master python scripts/image_gen.py "A cat" --backend openai
conda run -n ppt-master python scripts/image_gen.py "A cinematic portrait" --backend minimax
conda run -n ppt-master python scripts/image_gen.py "A product launch hero image" --backend qwen
conda run -n ppt-master python scripts/image_gen.py "科技感背景图" --backend zhipu
conda run -n ppt-master python scripts/image_gen.py "A product KV in cinematic style" --backend volcengine
```
...
```bash
conda run -n ppt-master python scripts/gemini_watermark_remover.py <image_path>
conda run -n ppt-master python scripts/gemini_watermark_remover.py <image_path> -o output_path.png
conda run -n ppt-master python scripts/gemini_watermark_remover.py <image_path> -q
```
...
```bash
conda run -n ppt-master pip install Pillow numpy
```
