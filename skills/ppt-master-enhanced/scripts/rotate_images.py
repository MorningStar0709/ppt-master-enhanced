...
Usage:
    conda run -n ppt-master python scripts/rotate_images.py gen <images_directory>
...
class ImageRotator:
    ...
    @staticmethod
    def _repo_root() -> Path:
        # scripts/rotate_images.py -> skills/ppt-master-enhanced/
        ...
        for f in files:
            if f.is_file() and f.suffix.lower() in valid_exts:
                try:
                    ...
                    # path is used for JSON data and must stay repo-relative
                    # e.g. "projects/Name/images/1.jpg"
                    try:
                        repo_rel_path = f.relative_to(repo_root).as_posix()
                    except ValueError:
                        print(f"[WARN] Warning: {f.name} is outside the repository root, skipped")
                        continue
...
def main() -> None:
    ...
    if command == 'gen':
        if len(sys.argv) < 3:
            safe_print("[ERROR] Error: images directory path is required")
            safe_print("Usage: conda run -n ppt-master python scripts/rotate_images.py gen <images_directory>")
            sys.exit(1)

        target_dir = sys.argv[2]
        try:
            output_path = rotator.generate_html_tool(target_dir)
            safe_print(f"[OK] HTML tool created: {output_path}")
            safe_print(f"[LINK] Open in browser: file:///{Path(output_path).as_posix()}")
        except Exception as e:
            safe_print(f"[ERROR] Generation failed: {e}")
            sys.exit(1)

    elif command == 'fix':
        if len(sys.argv) < 3:
            safe_print("[ERROR] Error: fixes.json path is required")
            safe_print("Usage: conda run -n ppt-master python scripts/rotate_images.py fix <fixes.json>")
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            safe_print("[ERROR] Error: images directory path is required")
            safe_print("Usage: conda run -n ppt-master python scripts/rotate_images.py auto <images_directory>")
            sys.exit(1)
        ...
        try:
            ...
            if count == 0:
                safe_print("[INFO] No images requiring automatic fix found")
        except Exception as e:
            safe_print(f"[ERROR] Automatic fix failed: {e}")
            sys.exit(1)

    else:
        safe_print(f"[ERROR] Error: unknown command '{command}'")
        safe_print(__doc__)
        sys.exit(1)
