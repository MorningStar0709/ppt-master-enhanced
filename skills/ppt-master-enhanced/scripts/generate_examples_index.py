...
Usage:
    conda run -n ppt-master python skills/ppt-master-enhanced/scripts/generate_examples_index.py
    conda run -n ppt-master python skills/ppt-master-enhanced/scripts/generate_examples_index.py examples
"""
...
try:
    ...
    try:
        from project_utils import find_all_projects, get_project_info, CANVAS_FORMATS  # type: ignore
    except ImportError:
...
def generate_examples_index(examples_dir: str = 'examples') -> str:
    ...
    if not examples_path.exists():
        safe_print(f"[ERROR] Directory not found: {examples_dir}")
        return ""

    safe_print(f"[SCAN] Scanning directory: {examples_dir}")

    # Find all projects
    projects = find_all_projects(examples_dir)

    if not projects:
        safe_print("[WARN] No projects found")
        return ""

    safe_print(f"Found {len(projects)} project(s)")
    ...
    content.append("> This file is auto-generated by `skills/ppt-master-enhanced/scripts/generate_examples_index.py`\n")
    ...
    content.append("```bash")
    content.append(
        "conda run -n ppt-master python -m http.server --directory examples/<project_name>/svg_output 8000")
    ...
    content.append("```bash")
    content.append(
        "conda run --no-capture-output -n ppt-master python skills/ppt-master-enhanced/scripts/project_manager.py init my_project --format ppt169 --dir examples")
    content.append("```\n")
    ...
    content.append("4. Directory naming format: `{project_name}_{format}`\n")

    content.append("### Submission Process\n")
    content.append("1. Create a project under the `examples/` directory")
    content.append(
        "2. Validate the project: `conda run --no-capture-output -n ppt-master python skills/ppt-master-enhanced/scripts/project_manager.py validate examples/<project>`")
    content.append("3. Update the index: `conda run --no-capture-output -n ppt-master python skills/ppt-master-enhanced/scripts/generate_examples_index.py`")
...
def main() -> None:
    ...
    safe_print("=" * 80)
    safe_print("PPT Master - Examples Index Generator")
    safe_print("=" * 80 + "\n")
    ...
    try:
        ...
        safe_print(f"   Contains {len(content.splitlines())} lines")

        # Display statistics
        projects_count = content.count('- **[')
        safe_print(f"   Indexed {projects_count} project(s)")
