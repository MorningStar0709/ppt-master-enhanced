...
| -------------------- | ------------------------------ |
| `clipPath` on non-image elements | Draw the target geometry directly |
...
Use `project_manager.py apply-template <project_path> medical_university` so the SVG/design files land in `templates/` and any template image assets land in `images/`.
